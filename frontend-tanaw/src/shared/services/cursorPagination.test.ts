...
describe("cursor page collection", () => {
  it("collects every page in cursor order", async () => {
    ...
    await expect(collectCursorPages(load, "activity log")).resolves.toEqual([1, 2]);
  ...
  it.each([
    ...
    await expect(collectCursorPages(async () => response as CursorPage<number>, "activity log")).rejects.toThrow(message);
  });

  it("rejects a repeated cursor before issuing an unbounded request loop", async () => {
    await expect(collectCursorPages(async () => page([], true, "same"), "activity log")).rejects.toThrow("repeated continuation cursor");
  });

  it("fails closed when unique cursors exceed the configured safety limit", async () => {
    let pageNumber = 0;
    await expect(
      collectCursorPages(async () => page([], true, `cursor-${++pageNumber}`), "activity log", { maxPages: 2 }),
