        ...
        >
          ...
          <header className="relative flex items-start justify-between gap-4 border-b border-emerald-100/80 bg-[linear-gradient(135deg,rgba(236,253,245,0.92)_0%,rgba(255,255,255,0.98)_54%,rgba(255,251,235,0.78)_100%)] px-6 py-5 max-sm:px-5">
            ...
            <div className="min-w-0">
              {eyebrow && <p className="mb-1 font-mono text-[10px] font-bold tracking-[0.18em] text-emerald-700/80 uppercase">{eyebrow}</p>}
              <h2 className="text-tanaw-navy text-xl leading-tight font-bold tracking-tight">{title}</h2>
            </div>
            <button
              type="button"
              aria-label="Close modal"
              onClick={onClose}
              className="hover:text-tanaw-green focus:ring-tanaw-green/15 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-emerald-100 bg-white text-slate-500 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:bg-emerald-50 focus:ring-4 focus:outline-none"
            >
              <X size={17} />
            </button>
          </header>
