...
export function parseUpgradeDetail(payload: unknown): ClientUpgradeRequiredDetail {
  const body = isRecord(payload) ? payload : {};
  const error = isRecord(body.error) ? body.error : {};
  return {
    message: typeof error.message === "string" ? error.message : "Refresh TANAW Portal to load the required release.",
