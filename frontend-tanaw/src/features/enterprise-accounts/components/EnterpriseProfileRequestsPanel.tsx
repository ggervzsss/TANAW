...
import { type AccountProfileChangeRequest, type AccountSummary, resolveEnterpriseProfileChangeRequest } from "@/shared/services/accountManagement";
...
export function EnterpriseProfileRequestsPanel({ accounts, canResolve, onAccountUpdated }: EnterpriseProfileRequestsPanelProps) {
  ...
  const resolutionMutation = useMutation({
    mutationFn: ({ enterprise, request, action }: ResolutionPayload) => resolveEnterpriseProfileChangeRequest(enterprise.id, request.type, action),
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "Unable to resolve profile change request.")),
  });
    ...
    <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50/70 p-4">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-amber-800">{canResolve ? "Review and apply verified enterprise profile changes." : "Visible for admin review. IT personnel apply or decline these requests."}</p>
        </div>
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {requests.map(({ enterprise, request }) => (
          <article key={`${enterprise.id}-${request.type}`} className="rounded-xl border border-amber-200 bg-white p-4 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs font-semibold text-slate-500">{request.label} change requested</p>
