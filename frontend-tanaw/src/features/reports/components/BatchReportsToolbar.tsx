...
export function BatchReportsToolbar({
  query,
  periods,
  selectedPeriodId,
  scopeType,
  ...
  onBarangayChange,
  onGenerate,
}: BatchReportsToolbarProps) {
  return (
    <div className="flex flex-wrap items-end gap-3 border-b border-gray-200 bg-gray-50 p-4">
      <label className="min-w-64 grow">
        <span className="mb-1 block text-[10px] font-bold tracking-wide text-slate-500 uppercase">Search reports</span>
        <span className="relative block">
          <Search size={14} className="absolute top-1/2 left-3 -translate-y-1/2 text-gray-400" />
          <input
            value={query}
            onChange={(event) => onQueryChange(event.target.value)}
            placeholder="Enterprise, site, or barangay"
            className="focus:ring-tgreen-dark w-full rounded-lg border border-gray-300 bg-white py-2 pr-4 pl-9 text-sm outline-none focus:ring-1"
          />
        </span>
      </label>
...
function SelectField({ label, value, onChange, disabled, children }: { label: string; value: string; onChange: (value: string) => void; disabled: boolean; children: React.ReactNode }) {
  return (
    <label>
      <span className="mb-1 block text-[10px] font-bold tracking-wide text-slate-500 uppercase">{label}</span>
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        disabled={disabled}
        className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 outline-none disabled:bg-slate-100"
      >
        {children}
      </select>
    </label>
