...
export function StaffAnalyticsPage() {
  ...
  return (
    <PageMotion>
      ...
      {(reportsQuery.isError || comparisonReportsQuery.isError || periodsQuery.isError) && (
        <p className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">Dashboard data could not be loaded. Please try again shortly.</p>
      )}
      ...
      <motion.section className="grid grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-4" variants={stagger}>
        ...
        <div className="flex flex-col justify-between rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <select
            ...
            className="focus:ring-tgreen-dark mt-4 w-full cursor-pointer rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:ring-1 disabled:bg-slate-100"
          >
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody>
                  {enterpriseRows.map((row) => (
                    <tr key={row.enterpriseId} className="border-t border-slate-100">
                      <td className="px-3 py-2 font-semibold">{row.enterpriseName}</td>
                      <td className="px-3 py-2 font-mono">{formatDecimal(row.entriesExact)}</td>
                      <td className="px-3 py-2 font-mono">{formatDecimal(row.uniqueEstimateExact)}</td>
                      <td className="px-3 py-2 font-mono">{row.reports.length}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function CoverageValue({ label, value, tone }: { label: string; value: number; tone: "emerald" | "amber" }) {
  return (
    <div className={`rounded-md border bg-white/60 px-2 py-1.5 ${tone === "emerald" ? "border-emerald-100 text-emerald-800" : "border-amber-100 text-amber-800"}`}>
