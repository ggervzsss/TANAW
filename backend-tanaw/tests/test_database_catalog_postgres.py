...
@pytest.mark.asyncio
async def test_public_catalog_matches_the_application_erd(
    catalog_connection: AsyncConnection,
) -> None:
    migration_head = await catalog_connection.scalar(
        text("SELECT version_num FROM alembic_version")
    )
    assert migration_head == DATABASE_SCHEMA_REVISION
...
@pytest.mark.asyncio
async def test_database_columns_match_models(
    catalog_connection: AsyncConnection,
) -> None:
    rows = (
...
@pytest.mark.asyncio
async def test_database_support_objects_match_the_declared_inventory(
    catalog_connection: AsyncConnection,
) -> None:
    sequences = set(
        await catalog_connection.scalars(
...
@pytest.mark.asyncio
async def test_nonpartition_indexes_match_model_metadata(
    catalog_connection: AsyncConnection,
) -> None:
        ...
        for constraint in table.constraints:
            if isinstance(constraint, PrimaryKeyConstraint):
                expected_indexes.add(
                    str(constraint.name) if constraint.name else f"{table.name}_pkey"
                )
