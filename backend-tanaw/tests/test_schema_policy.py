...
from app.db.schema_policy import (
...
def test_autogenerate_excludes_reflected_partition_children_and_their_indexes() -> None:
    ...
    assert not include_schema_object(child, child_name, "table", True, None)
    assert not include_schema_object(child_index, "generated_index", "index", True, None)


def test_autogenerate_includes_modeled_and_unrecognized_relations() -> None:
    registry = SimpleNamespace(name="site_telemetry_rollup_partitions")
    unmanaged = SimpleNamespace(name="unexpected_table")

    assert include_schema_object(registry, registry.name, "table", True, None)
    assert include_schema_object(unmanaged, unmanaged.name, "table", True, None)
    assert include_schema_object(unmanaged, unmanaged.name, "table", False, None)


def test_alembic_commits_each_irreversible_revision_atomically() -> None:
    environment_source = (Path(__file__).resolve().parents[1] / "alembic" / "env.py").read_text(
        encoding="utf-8"
    )

    assert environment_source.count("transaction_per_migration=True") == 2
