...
def include_schema_object(
    ...
    """Exclude only dynamic partition children from Alembic schema comparison.

    The parent and registry are modeled application tables. Monthly children are
    created and retired by the retention service, so treating them as static
    schema objects would make every catalog comparison time-dependent.
    """
