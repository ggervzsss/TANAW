...
async def ingest_desktop_telemetry(
    ...
    await broadcast_summary(db, account)
...
async def generate_final_report(
    ...
    await broadcast_summary(db, actor)
    await record_operational_log(
        db,
        category="Staff Operation",
        severity="Success",
...
async def update_alert_status(
    ...
    return to_operational_alert_summary(alert)
...
async def broadcast_summary(db: AsyncSession, account: Account) -> None:
    summary = await get_operational_summary(db, account)
    await operational_ws_manager.broadcast(
