...
class LocalReportOutboxTest(unittest.TestCase):
    ...
    def test_lost_ack_restart_and_replay_return_the_original_revision(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                restarted_store.list_local_reports()[0]["sync_status"],
                "synced",
            )
    ...
    def test_new_request_creates_an_immutable_next_revision(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            second = store.create_local_report_revision(
                "REP-JUNE",
                JUNE_PERIOD_ID,
                notes="corrected",
            ...
            self.assertEqual(store.list_local_reports()[0]["notes"], "corrected")
            self.assertEqual(
                store.list_local_reports()[0]["sync_status"],
                "pending_cloud_sync",
            )
            self.assertEqual(
    ...
    def test_dead_letter_does_not_block_another_ready_report(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            july = store.create_local_report_revision("REP-JULY", JULY_PERIOD_ID)
