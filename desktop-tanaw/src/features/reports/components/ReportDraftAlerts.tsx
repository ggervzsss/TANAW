...
export function ReportDraftAlerts({ activeReport, isError, isReadOnly, metricsError, validationError }: ReportDraftAlertsProps) {
  return (
    <>
      ...
      {validationError && !metricsError && !isError && !isReadOnly && (
        <div className="mb-5 flex items-start gap-2 rounded-sm border border-red-200 bg-red-50 p-4 shadow-inner">
          <AlertTriangle size={16} className="mt-0.5 shrink-0 text-red-600" />
          <div>
            <h4 className="mb-1 text-[10px] font-bold tracking-wider text-red-800 uppercase">Report Validation</h4>
            <p className="text-xs leading-relaxed text-red-700">{validationError}</p>
          </div>
        </div>
      )}
    </>
