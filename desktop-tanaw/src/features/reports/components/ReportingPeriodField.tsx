import { formatReportingPeriodLabel } from "../utils/reporting-period";

type ReportingPeriodFieldProps = {
  description: string;
  isLoading?: boolean;
  label: string;
  period: string;
};

export function ReportingPeriodField({ description, isLoading = false, label, period }: ReportingPeriodFieldProps) {
  const periodLabel = formatReportingPeriodLabel(period);

  return (
    <div>
      <label className="mb-2 block text-xs font-semibold tracking-wider text-[#111827] uppercase">{label}</label>
      <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]" aria-busy={isLoading}>
        <p className="text-sm font-bold text-[#111827]">{isLoading ? "Loading reporting period..." : periodLabel}</p>
        <p className="mt-1 text-xs leading-5 text-gray-500">{description}</p>
      </div>
    </div>
  );
}
