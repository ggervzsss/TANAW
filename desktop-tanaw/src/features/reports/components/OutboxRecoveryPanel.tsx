    ...
    <Card className="overflow-hidden rounded-sm border border-amber-200 bg-amber-50/40 shadow-sm" data-testid="report-delivery-recovery">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-amber-200 px-5 py-4">
        <div className="flex gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold tracking-wider text-amber-950 uppercase">Report delivery recovery</h3>
            <p className="mt-1 max-w-3xl text-xs leading-5 text-amber-900">
              These exact official revisions remain safely stored on this device. Review the failure before deliberately requeuing it; official submissions cannot be discarded.
            </p>
          </div>
        </div>
      ...
      {error && <p className="border-b border-amber-200 px-5 py-3 text-xs font-semibold text-red-700">Recovery status unavailable: {error}</p>}
            ...
            <section key={item.outbox_item_id} className="grid gap-4 bg-white/70 px-5 py-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.7fr)]">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="font-mono text-xs font-bold text-[#111827]">{item.report_id}</p>
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase ${item.status === "dead_letter" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-800"}`}>
                    {item.status === "dead_letter" ? "Intervention required" : "Retry scheduled"}
                  </span>
                </div>
                <p className="mt-1 text-xs text-gray-600">
                  Revision {item.revision_number} · {item.attempt_count.toLocaleString()} recorded attempt{item.attempt_count === 1 ? "" : "s"}
                </p>
                <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2">
                  <div>
                    <dt className="font-semibold text-gray-500">Failure class</dt>
              ...
              <div className="self-center">
                ...
                >
                  {isRetrying ? "Requeuing…" : "Requeue exact revision"}
                </button>
              </div>
            </section>
          );
