...
const executable = path.resolve(
  ...
  "dist",
  "tanaw-ml-service",
  process.platform === "win32" ? "tanaw-ml-service.exe" : "tanaw-ml-service",
);
...
if (bundleBytes > maximumBundleBytes) {
  throw new Error(
    `The bundled ML runtime is ${(bundleBytes / 1024 ** 3).toFixed(2)} GiB; ` +
      `the ${process.platform} limit is ${(maximumBundleBytes / 1024 ** 3).toFixed(0)} GiB.`,
  );
}
const child = spawn(executable, [], {
  env: {
    ...process.env,
    TANAW_APP_DATA_DIR: path.resolve("ml-service", "build", "runtime-smoke-data"),
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (response.ok) {
        ...
        if (payload.api_contract_version !== 9) {
...
if (!verified) {
  throw new Error("The bundled ML runtime did not pass its offline health check.");
}
